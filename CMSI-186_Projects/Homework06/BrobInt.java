   ...
   public BrobInt( String value ) {
    ...
    if(validateDigits()) {
      ...
      if(lastChar == '+' || lastChar == '-') {
        len = reversed.length()-2;
      } else {
        len = reversed.length()-1;
      }
      int[] intValue = new int[len];
      for(int i=0; i<=len; i++) {
        intValue[i] = Integer.parseInt(String.valueOf(reversed.charAt(i)));
      }
    }
   }
   ...
   *  ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ */
   public boolean validateDigits() {
    ...
    for(int i=0; i<internalValue.length(); i++) {
      if(!(internalValue.contains(String.valueOf(valid.charAt(i))))) {
        return false;
      }
    } return true;
   }
